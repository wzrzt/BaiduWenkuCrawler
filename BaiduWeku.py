        ...
        for pure_addr in url_list:
            ...
            try:
                ...
                for body_info in all_body_info:
                    # 改进方法，all_body_info中的每个元素既包含正文又包含位置，xyz值，
                    # 所以当y增加时先加一个换行符上去
                    # 这样也有问题，比如有些不应该分段的地方。这个时候再判断一下结尾标点

                    # print(y_tmp)
                    # print(body_info["p"])
                    # print(type(body_info["p"]))
                    try:
                        # text_result = body_info["c"].strip()
...
# 作者：木子昭
# 链接：https://www.jianshu.com/p/8c103a566bd9
# 來源：简书
